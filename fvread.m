function [features_vectors, labels, inputfiles] = fvread(template,niftilist)

% fvread: reads in raw nifti files and labels for use with fv package
% USAGE:  fvread('template.img','niftifile.txt')
% INPUT:  template is a template .nii or .img file, binarized, which will be used
...
% ** Be sure to edit the paths to your nifti tools and images below!
...
% Check to make sure we have 3D data
    if  isempty(regexp(template,'.nii.gz','end'))==0
        ...
        [inputfiles, labels] = textread(niftilist,'%s\t%d\n');
